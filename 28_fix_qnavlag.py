#!/usr/bin/env python3
"""28_fix_qnavlag.py: qnav + 可学习残差滞后 τ_res (先验120s) — 压湿态闭环中段追赶

背景: qnav 全PASS但湿G1中段锚点超 (300s Δ0.337/420s Δ0.299 — 残差中段追赶过快)。
τ_res 延迟残差响应 → 中段追赶变慢 → 锚点回落; 前段已由蒸发缓冲主导 (τ_res 只微调)。

预注册 (冻结 2026-08-17):
  F1: 湿态闭环 G1 中段: 300s |Δ|≤0.15 ∧ 420s |Δ|≤0.15
  F2: 湿态开环 τ63∈[240,900] 保持 (qnav 480s 不丢)
  F3: rollout 2seeds mean ≤ 3.5 (qnav 2.463, 慢化预算)
  F4: 湿态 sh1_out 首步偏差 ≤8°C 保持 (qnav 2.79)
  F5 (审计): τ_res 学得值 + 前段锚点保持 (60/120/180s 的 0.039/0.132/0.119 不恶化)
裁决: F1∧F2∧F3∧F4 = PASS
"""
...
class QnavLag(nn.Module):
    ...
    def tau_res(self):
        return TAU_RES0 * F.softplus(self.raw_res)


def integrate_qnavlag(model0, mod, exo, h, Tm, rB, m1, m2, steps, z_lag=None):
    M = model0.tri("M")[:, None]
    ...
    for t in range(steps):
        ...
        for _ in range(t02.N_SUB):
            ...
            Tm0_in = (k_t[0] * rB / 3600.0 + UA[0] * ts[0] - q_w1 + z_lag[0]) / Cm[0]
            Tm1_in = (k_t[1] * rB / 3600.0 + UA[1] * ts[1] - q_w2 + z_lag[1]) / Cm[1]
            Tm2_in = (k_t[2] * rB / 3600.0 + UA[2] * ts[2] + z_lag[2]) / Cm[2]
    ...
    return out, h, Tm, rB, m1, m2, z_lag


def train_qnavlag(df, seed, fast=False):
    ...
    model0.eval()
    mod = QnavLag().to(DEVICE)
    # 热启动: mlp 从 qnav 载入, raw_res 保持先验
    qnav_state = torch.load(os.path.join(OUT, "model_res_qnav_seed0.pt"),
                            map_location=DEVICE, weights_only=True)
    own = mod.state_dict()
    for k, v in qnav_state.items():
        if k in own and own[k].shape == v.shape:
            own[k] = v.clone()
    mod.load_state_dict(own)
    ...
    def fwd(exo_t, init_t, obs_t):
        ...
        out, *_ = integrate_qnavlag(model0, mod, exo_t, h, Tm, rB, m1, m2,
                                    exo_t.shape[1])
        return out
    ...
    torch.save(best_state, os.path.join(OUT, f"model_res_qnavlag_seed{seed}.pt"))
    print(f"[qnavlag s{seed}] {n_ep_done}ep val={best_va:.3f} "
          f"τ_res={mod.tau_res().item():.1f}s", flush=True)
    return mod, best_va
...
def main():
    ...
    summ = {"train": {}, "judge": {}}
    ckpts = [os.path.join(OUT, f"model_res_qnavlag_seed{sd}.pt") for sd in (0, 1)]
    if all(os.path.exists(c) for c in ckpts):
        print("[skip] 2 checkpoints exist", flush=True)
    else:
        for sd in (0, 1):
            _, va = train_qnavlag(df, sd)
            summ["train"][str(sd)] = {"val_mse": round(va, 4)}

    mod0 = QnavLag().to(DEVICE)
    mod0.load_state_dict(torch.load(ckpts[0], map_location=DEVICE, weights_only=True))
    mod0.eval()
    for p in mod0.parameters():
        p.requires_grad_(False)
    print(f"[qnavlag s0] τ_res={mod0.tau_res().item():.1f}s", flush=True)
    ...
    with torch.no_grad():
        for r in wet_idx:
            ...
            out, *_ = integrate_qnavlag(model0, mod0, exo_t, h, Tm, rB, m1, m2, 1)
            errs.append(float(out[0, 0, 1] - T_all[r + 1, 1]))
    bias = float(np.mean(np.array(errs)))
    print(f"[F4] wet sh1_out bias={bias:.2f}°C (gate 8)", flush=True)

    # F3: rollout (2 seeds)
    rolls = []
    for sd in (0, 1):
        mod = QnavLag().to(DEVICE)
        mod.load_state_dict(torch.load(ckpts[sd], map_location=DEVICE, weights_only=True))
        mod.eval()
            ...
            for t in range(t02.ROLL_STEPS):
                ...
                if t == 0:
                    ...
                    h, Tm, rB, m1, m2 = r26.init_states_evap(
                        model0, torch.tensor(row, device=DEVICE)[None, :],
                        torch.tensor(obs, device=DEVICE)[None, :])
                out, h, Tm, rB, m1, m2, z_lag = integrate_qnavlag(
                    model0, mod, exo_t, h, Tm, rB, m1, m2, 1)
                preds[t] = out[0, 0].cpu().numpy()
        truths = T_all[START: START + t02.ROLL_STEPS]
        rmse = float(np.sqrt(np.mean((preds[:, 4] - truths[:, 4]) ** 2)))
        rolls.append(rmse)
        print(f"[qnavlag s{sd}] rollout={rmse:.3f}", flush=True)
    ...
    def run_step(row_idx, d_v2, W_mul):
        ...
        h, Tm, rB, m1, m2 = r26.init_states_evap(
            model0, torch.tensor(row, device=DEVICE)[None, :],
            torch.tensor(obs, device=DEVICE)[None, :])
        out, *_ = integrate_qnavlag(model0, mod0, exo, h, Tm, rB, m1, m2, N)
    ...
    for name, row_idx, state, power in (("wet", OP_WET, "wet", 332.85),
        ...
        with torch.no_grad():
            for t in range(N):
                ...
                out, h, Tm, rB, m1, m2, z_lag = integrate_qnavlag(
                    model0, mod0, exo, h, Tm, rB, m1, m2, 1)
    ...
    F1 = bool(summ["loop_wet"]["G1_deltas"] is not None
              and summ["loop_wet"]["G1_deltas"][3] <= 0.15
              and summ["loop_wet"]["G1_deltas"][4] <= 0.15)
    F2 = bool(summ["step_wet"]["tau63_s"] is not None
              and 240 <= summ["step_wet"]["tau63_s"] <= 900)
    F3 = bool(np.mean(rolls) <= 3.5)
    F4 = bool(abs(bias) <= 8.0)
    judge = {"F1": F1, "F2": F2, "F3": F3, "F4": F4,
             "rollout_mean": round(float(np.mean(rolls)), 3),
             "tau_res": round(float(mod0.tau_res().item()), 1),
             "verdict": "PASS" if (F1 and F2 and F3 and F4) else "FAIL"}
    summ["judge"] = judge
    with open(os.path.join(OUT, "qnavlag_summary.json"), "w") as f:

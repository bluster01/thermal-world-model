"""A4+A6 joint scheduling arm (2026-08-28, user-ordered).

User sequence: A4 (k load schedule, -7.6%, first -5% passer but Q3 +13.5%)
then A6 (pressure-tau schedule, H18 +1.2%, Q3 target missed) then THIS joint.

Rationale (pre-registered): A4 acts on the FUEL-side channel (k: combustion ->
metal heating), A6 on the TRANSPORT-side channel (tau: flow+pressure -> lag).
Different physical paths, in principle complementary: A4's gains sit at the
load ENDS (Q1 -16%, Q4/Q5 -10~12%) where energy mismatch is largest; A6's
pressure dimension addresses pressure-flow decoupling. The joint tests whether
the two schedules add without re-pooling (A9 showed parameter-pool
reallocation is the usual failure mode).

Schedule structure (both, learnable, all start OFF):
  tau: inv = s^(-a_tau) * (p/p_ref)^(-b_tau), applied to tau_b/tau_mix1/2
       (free-arm parity: tau_b scheduling is load-bearing, A9)
  ua:  ua *= s^(+a_ua)
  k:   k_t *= exp(tanh(w*(s-1))*ln(k_max)), k_max=1.5
Params: alpha_tau_raw=-4, alpha_ua_raw=-4, beta_tau_raw=-4, k_w_raw=0.

Baseline pair (same protocol, seed0, corrected v2.1, grid props):
  baseline 0.4840 | A4 0.447 | A6 0.490 | joint: TBD
"""
...
OUT = P / "a46_joint_schedule_probe"
...
class JointScheduledTransition(Fan2020UDETransition):
    """A4 (k) + A6 (flow-pressure tau) schedules in one transition."""
    ...
    def _substep(self, h, tm, rb, m1, m2, lag1, lag2, dsw1, dsw2, d_flow, u_b,
        ...
        if self.alpha_scale != 0.0:
            ...
            k_t = k_t * torch.exp(
                torch.tanh(self.k_w_raw * (s - 1.0)) * float(np.log(K_MAX))
            ).unsqueeze(-1)
...
def _promote(model, alpha_scale: float = 1.0):
    ...
    tr.__class__ = JointScheduledTransition
    ...
    if "alpha_tau_raw" not in tr._parameters:
        ...
        tr.register_parameter("beta_tau_raw",
                              nn.Parameter(torch.tensor(-4.0, device=dev)))
        tr.register_parameter("k_w_raw",
                              nn.Parameter(torch.tensor(0.0, device=dev)))
    return model
...
def main():
    ...
    if args.sanity:
        ...
        tr.alpha_tau_raw.data.fill_(float(np.log(1.0 / 2.5) - np.log(1 - 1.0 / 2.5)))
        tr.beta_tau_raw.data.fill_(float(np.log(0.8 / 1.5) - np.log(1 - 0.8 / 1.5)))
        tr.k_w_raw.data.fill_(-0.12)
        with torch.no_grad():
    ...
    if args.train:
        ...
        print(f"[a46] H18 ch4 overall={np.mean(ch4['bin_means']):.3f} "
              ...
              f"b_tau={b_tau.item():.3f} w={w:+.3f}", flush=True)
        (OUT / "report.json").write_text(json.dumps(
            {"arm": "a46_joint_schedule",

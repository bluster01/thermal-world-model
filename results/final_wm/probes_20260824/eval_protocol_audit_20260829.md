...
## 5. OOF 结果与 σ 分解（2026-08-29，11,783 窗全 val stride 9）

`zcond_oof_sweep.py` → `oof_sweep/oof_report.json`。三臂同窗：

| 模型 | H18 | 分箱极差 | NLL | (err/σ)² 项 | logσ² 项 | σ/err |
|---|---|---|---|---|---|---|
| 对照 | 0.4340 | 1.793 | 1.002 | 0.640 | 0.362 | 1.34 |
| A (k-only) | 0.5563 | 1.947 | 1.292 | 0.758 | 0.534 | 1.24 |
| B (k+τ+UA) | 0.4306 | 1.787 | **0.989** | **0.682** | **0.307** | 1.27 |

**读数：**

1. **256 窗协议样本系统性乐观**：B 在协议窗 −3.6%，全量 val 只有 **−0.78%**
   （对 OOF 对照 0.4340）——256 窗样本把 B 的优势放大约 4.6×。A 的 REJECT
   双口径一致（+22.7% / +28.2%），不受影响。→ 单窗 SE≈0.008°C 的估计
   低估了窗口异质性；**幅度接近噪声带的结论应以全量 OOF 为准**。
2. **NLL 降而 MAE 不降的机制已坐实（§2 的假设）**：B 的 NLL −1.3% 全部来自
   logσ² 项（−15%），(err/σ)² 项**反升 +6.5%**——B 把 σ 收得更紧（σ/err
   1.34→1.27），收益是校准性不是点精度。zcond A 的 NLL 退化（1.292）也是
   σ 项主导（0.534 vs 对照 0.362）。
3. **z 路线最终裁定（证据层）**：B 的"增益"在全量上不稳健；z1 正交于负荷
   （r²=0.033）是机制上值得记录的现象，但没有转化为可测的 H18 改善。
   原始命题（encoder 特征 > 单一标量）的最终形态：**有信息（正交维存在）、
   无收益（全量 −0.8%，噪声级）**，INCONCLUSIVE，勿升写为收益。

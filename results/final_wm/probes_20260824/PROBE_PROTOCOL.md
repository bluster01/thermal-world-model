...
- **串联多臂用 `kill -0 <PID>` 等待，禁止 `pgrep -f "<模式>"`**：链条 shell 自身的
  ...
  1.6-2.9 小时）。
  **二次违反（同日）**：A1 臂用 `pgrep -f "lpv_schedule_probe.py --train --pin-prior"`
  等待，同样自匹配，**空转 3.8 小时未启动**——本条协议已存在仍复犯。
  **推论并入本条**：`pgrep -f` 也不可用于**查状态**（`pgrep -f "ua-physics"` 会匹配链条
  shell 而误报"运行中"）。查状态一律用 **ledger 文件是否存在 + epoch 进度**。
  最稳做法：写成顺序命令 `cmd1; cmd2`（同一进程内串联），不写等待循环。

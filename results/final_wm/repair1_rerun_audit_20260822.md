...
## 附记（2026-08-22 16:0x 本地审计）：O1 重跑回传，F1 闭合

执行侧 1ab13c8 完成 O1 三臂×3 全量重跑（44eda5875 栈，含 resume 洞修复）：
9/9 全新鲜（epochs=30、commit 戳正确、无续跑标记）。判决修复——

| 臂 | v0.2（陈旧作废） | 修复①栈重判 |
|---|---|---|
| learned | MIXED | **SUPPORTED 3/3**（改善 +0.305/0.218/0.273，13 天块自助 CI 下界 0.16-0.27 全 >0.05 门） |
| hybrid | REJECTED | **SUPPORTED 3/3**（+0.241/0.241/0.243，CI 下界 0.21+） |
| steady 参考 | — | val NLL 2.316-2.325 |

即：锚定相对观测器（①-B）把"学习修正"从 v0.2 的净伤害翻转为稳定净收益，
hybrid 三 seed 改善幅度异常一致（0.241±0.001）。A2 证件由 ❌ 转 ✅。
**次生修复确认**：其提交说明指 matrix_summary 缺 t1/r1 块（8/18 覆写遗留）；
本地已据本审计引用的同一新鲜产物 `r1_report.json` 重建 summary 的 r1 块
（t1 块按减臂设计本就无判决，留待 v0.4 终审后重发）。
F1（resume 洞）闭环：修复+回归测试+执行侧复验三重确认。

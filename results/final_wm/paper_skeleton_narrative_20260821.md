...
**拍 1.5 · 为什么不能走 RL（领域定位，用户 08-21 补充）**
机器人与自动驾驶的世界模型靠交互式学习：在线 RL 探索动作空间、离线 RL 复用交互日志或仿真。
两条路在这里都封死：**在线**被安全等级禁止——过热器超限直接跳机，喷水阀的试错抖动（离散探测动作）
运行上不可接受；**离线**被反向 transfer gap 封死——锅炉强耦合、多扰动、时变，能支撑控制策略
迁移的仿真器不存在。**唯一可采的数据是闭环运行记录**。因此本文的研究问题比通用 WM 评估更尖锐：
*如何在反馈生成的数据上辨识动作通道的因果方向与幅度，当控制器把阀门系统性地耦合到它正在
驱动的温度上（共因结构）*——共因剥离本身就是本研究价值。
（落位：§1 拍 1 与拍 2 之间的独立段；§5 定位句"first qualification protocol aimed at the
setting RL cannot serve"）

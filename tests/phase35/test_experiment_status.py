...
def test_repository_registry_is_valid_and_has_one_active_gate():
    ...
    assert report["linux_authorized_gate"] == "final_world_model_pipeline"
...
def test_status_cli_emits_machine_readable_summary():
    ...
    assert payload["linux_authorized_gate"] == "final_world_model_pipeline"
...
def test_registry_accepts_test_authorized_active_gate():
    ...
    registry["experiments"]["jepa_b_series"]["status"] = "planned"
    registry["experiments"]["jepa_b5"]["status"] = "planned"
    registry["experiments"]["final_world_model_pipeline"]["status"] = "planned"
    report = module.validate_registry(registry, ROOT)
    assert report["valid"] is True

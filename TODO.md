...
> 更新：2026-08-11。本文是项目唯一人工任务队列；机器状态见 `configs/phase3_5/experiment_registry.json`。Gate C 真实 RM0-B 证明 baseline anchor 有效，但四路线预测近同而 response 幅值相差约 2.5 倍，动作分解仍不唯一。下一步固定 A1phys 做真实 RM1-A attribution；合成只作理论与负控制，Linux、test 和 MS4 仍禁止。旧 E1–E5 已废弃，Phase 4 暂停。
...
| **MS3-R** | 点位辨识、分支归因与真实模型扩充 | ▶ **GATE-C LOCAL REAL RM1-A ATTRIBUTION** | 固定A1扫描free容量/调度/local supervision，Linux 无任务 |
...
| 本地 / Codex | Linux 远端 |
|---|---|
| 设计/实现真实 1/100 RM1-A attribution；先固定A1，不做operator排名 | 当前无授权任务；不得自行训练、重跑 RM0-A/B/Gate B、访问 test 或启动 MS4 |
...
| 19 | 固定 A1phys 的真实 RM1-A attribution | 本地 | ▶ paired-free/additive/scheduled small-base-large/terminal-only |

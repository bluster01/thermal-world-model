...
MS2-J 已完成。当前只运行 MS2-D1 pure-delay validation：6 candidates × 3 seeds = 18 runs。D1 结果返回并经本地复算前不启动 D2，也不访问 test。
...
| MS2-D1 纯迟延 | R50+调度 truth 加 20 s delay | delay module 是否必要？响应和参数能否分别恢复？ | 6 candidates×3 seeds；oracle；learned vs no-delay；参数诊断分列 | ▶ 当前 Gate；按注册表授权 Linux |

## 当前任务更新：使用Linux现有数据开始45次拟合（2026-09-24）

- [x] 作者选择直接使用Linux已建合并表数据包。
- [x] 接入 `d4a04eb` 的18文件数据身份，模型、损失、种子与预算保持原样。
- [x] 新入口23项合成检查通过；原生传包流程停止。
- [ ] Linux领取 `unified_industrial_merged_stage1_20260924`，预运行后接续45次正式拟合。
- [ ] 回传公开进度及私有结果状态，按任务和种子汇总模块贡献。

直接执行：[新Linux入口](experiments/unified_merged_20260924/README.md)。本轮以此任务为准，原生数据版本保留为历史发布记录。
...
- [x] 完成本地合成合同/相关测试、保存 [验证记录](experiments/fmts_core_20260914/VERIFICATION.md) 和 [Linux推理执行单](experiments/fmts_core_20260914/RUN_LINUX.md)。
- [x] [核实候选使用史](analysis/fmts_greybox_core_20260914/EXPOSURE_REVIEW_ZH.md)：同源3–5月数据已参与历史拟合/分析，不能称全新独立test；评分保持关闭。
- [x] 权限更新后完成release同步/最终复测：35项工程测试通过，元数据预检确认已使用候选不适格为全新独立test。
- [x] 准备CORE1发布提交及 [证据计划](experiments/fmts_core_20260914/EVIDENCE_PLAN_ZH.md)，经origin/main交付Linux；不把发布状态当运行回执。
- [ ] Linux执行15个真实数据推理单元，无新训练；当前尚无启动/回传回执。
- [ ] 回传后核对30份预测/响应数组及15份状态轨迹，再分析组件依赖；不按响应符号挑选baseline，不自动重辨识。
- [ ] 如继续时间外探索，另注册23通道适配、最终模型和窗口，披露历史使用；独立确认需要另核实未用数据。

机器状态：[CORE1](experiments/fmts_core_20260914/experiment_state.json)。

## 上一任务：M7 / GNR1 审计完成（2026-09-14）
...
- [x] Linux `15757e8` 返回 M7R1 的 3 seed / 6 预测响应回放 / 9 健康回放，以及 GNR1 的 3 seed / 6 回放。
- [x] 保存 [审计与论文整合建议](docs/fmts2026/audits/m7_gnr1_20260914/REVIEW_ZH.md)：M7 改善旧 token 但对 GRU 0/3 不晋级；GNR 符号转负但响应幅值仍弱、未校准。相关测试重跑 24 通过。
- [ ] 据审计整合中英文稿/图：保留 GRU，使用 no-rewet 灰箱作匹配参考，附录完整保留旧灰箱及 token/M7 修复结果；v0.2 PDF 先保留快照。
- [ ] 作者审读响应幅值/现场校准边界；不默认追加搜索、重训、去 oracle 或访问 locked test。
...
- [x] 3 seed / 6 数组 Linux 回放及本地保存数组复算完成；no-rewet 响应符号改变但幅值仍弱，精度未显示改善。
...
- [ ] 如作者后续授权，分别考虑锁定测试、现场响应校准与去 oracle；再润湿消融 GNR1 已执行审计，不自动追加实验。
...
| 61 | 侧B完整复现矩阵（做实结论） | 本地+Hermes | ✓ 执行侧 b199d8d 全量回传（21 runs，0 RESUMED；commit 混合 0922cac/1b4f874 已核实=纯 scripts 工具提交，训练完整性无损）；设计侧审计 results/final_wm/sideB_replication_audit_20260824.md。**跨侧复现成立：D-SYN PASS×3、R1 三分臂排他性（physics_only/intact REJECTED，norew SUPPORTED 3/3 且冷却更强 −0.39..−0.52）、F3 润湿归因（+0.058→−0.293）、O1 learned/hybrid SUPPORTED（hybrid 幅度减半）、泄漏 16-shuffle 三清、单调性/校准量级一致**。**跨侧不一致（科学发现非事故）：T1 闭包精度增益侧B SUPPORTED 3/3（intact 6.8-9.8%、norew 8.8-11.1%）vs 侧A 不支持——闭包精度价值侧依赖，论文按侧分报禁止聚合**；生产臂 norew 维持（两侧唯一方向认证 + 侧B 精度增益最大臂）。T1 判决由设计侧只读重放落档（scripts/t1_verdict_sideB_replay.py → results/final_wm/t1_verdict_sideB_replay_20260824.json） |

| 62 | 黑箱基线包（canonical 协议下最终精度对比） | 本地 | ✓ 全量 13 组闭合（lstm/gru/dlinear/itransformer ×3 + n4sid_ridge），产物 results/final_wm/v05_blackbox_comparison_20260823.json（增量落盘+resume-skip，两次进程死亡零损失：23:50 shell 回收→修补；次日 OMP libiomp5md 冲突杀 n4sid→KMP_DUPLICATE_LIB_OK 重跑补齐）。**主汽温 H18 MAE 中位（°C）：iTransformer 0.362 / N4SID 0.344 / GRU 0.472 / LSTM 0.531 / DLinear 0.929；世界模型：physics_only 0.528 / intact 0.580 / norew 0.599（seed0 1.05 异常值）——"精度超纯黑箱"不成立，最优黑箱低 ~40%；与 LSTM/GRU 属平价档（H1 上 norew 0.12-0.28 与 iTransformer 0.13-0.14 持平，差距随 horizon 拉大）**。**响应方向（frac_negative，正确=1.0）：norew 双阀 1.0×3 seed 独占持证；LSTM/GRU/DLinear 双阀 0.000×3 全反号；iTransformer v1 0.13-0.41 / v2 0.19-0.66 随机；N4SID 无线性探针 n/a**。**最终口径：同 oracle 信息集下主汽温精度黑箱全线领先（执行侧 eec04f9 分箱探针：五负荷箱 1.4-2.3×，低负荷最大；公平性不对称 InstanceNorm/单通道两条成立待公平化探针，但不改当前可声称口径）+ 干预响应方向独占持证（黑箱 0/6 臂过方向门）**。执行侧探针批次审计 results/final_wm/probe_audit_20260825.md：数字全一致、v05 一行热修审过、闭环地形（恒工况收敛 571±3°C / 快斜坡失控 635/528°C=closure 联合支持域外外插）作信任边界证据、开环 670°C clamp 增益伪零记数值伪影 |

| 63 | v0.6 通道扩充修正案（backlog，8/30 后 CF/AE 窗口） | 本地+现场 | **已批准登记，冻结期内不启动**。诊断依据：sh1_in H18 MAE 4.5-5.1°C 已近信息集可预测性极限（within_bin_sigma 15.7°C、T0 一致性 1.25×，波动主源=未观测炉膛侧吸热：coal_command 是指令非实际热输入、无烟气侧通道——contracts.py BOUNDARY_ELEMENTS 无 flue/reheat/per-valve flow）；sh1_out 有 −3.6°C 结构性偏差（AE 候选，纯模型侧可修）。**优先级排序：① 再热进出口汽温作边界输入（炉膛出口烟温场代理，直击 sh1_in）；② 分级减温水流量测点（直接监督喷水混合项→闭包可辨识性升级，CF 因果声称前提）；③ sh1_out AE 偏差校正（零新数据）；④ 烟气温度/氧量；⑤ 再热作联合目标（新物理模块，最后做）。**触发条件=canonical 记录 v2 + 双侧全臂重训重判，论文局限性节可引"聚合精度代价主因=未观测炉膛侧驱动，已识别补点方案"**。**2026-08-25 执行侧数据资产发现（eec04f9）：all_merged_10s.csv 3.8GB/381 列/139 天含磨×8、给煤×8、烟气氧量烟温、8角配风、管壁温、吹灰、分级减温阀位——v0.6 从"需现场补点"降级为纯管道工程；sh1_inlet 信息缺失病因升级为数据侧可验证假说**。**Phase 1 已实施（用户批准先做 Phase 1，重训后续）：规格 docs/plans/2026-08-25-v06-canonical-v2-spec.md + 映射 configs/final_wm/channel_mapping_v2.json + src/final_wm/data_v2.py（boundary_ext 7 + aux 15 + mill_on 8；对齐门=网格包含[边缘裁边≤60行/内部 fail-closed]+7 通道数值交叉核验 corr≥0.99-0.999；质量门=覆盖/量程/冻结）+ 执行入口 build_canonical_v2.py + 9 项契约测试（全套 141/141）。本地首建双侧成功：n=707,709、对齐 7/7 corr=1.0000、final_outlet 自动选侧 A→左/B→右镜像正确、质量门全过；v1 六键逐位搬运已断言。npz 不入 git（执行侧凭 meta 三 sha256 确定性重建）。待办：Phase 0 磨切换事件研究（待批）、Phase 2 A1-A4 修正案（8/30 后）** |

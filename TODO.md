...
> 更新：2026-08-11。本文是项目唯一人工任务队列；机器状态见 `configs/phase3_5/experiment_registry.json`。Gate C 真实 RM1-A 已完成：容量扫描未见 response 被 free 单调吞噬，terminal-only 以 9.41× persistence 的错误局部状态换取末温拟合，故局部监督必须保留。下一步设计 RM2 日块/rolling-fold 稳健性批；合成只作理论与负控制，Linux、test 和 MS4 仍禁止。旧 E1–E5 已废弃，Phase 4 暂停。
...
| **MS3-R** | 点位辨识、分支归因与真实模型扩充 | ▶ **GATE-C RM2 DESIGN** | RM1-A已审计；设计日块/rolling-fold稳健性门，Linux 无任务 |
...
| 20 | RM2 日块响应与 rolling-fold 稳健性设计/代码 | 本地 | ▶ 先设计与 smoke；暂不授权 Linux |

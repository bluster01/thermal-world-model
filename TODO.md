...
| 54 | R1 三 seed 判决审计：seed1 leakage 边际案 | 本地+Hermes | 判决在档 **REJECTED**（方向门 3/3 PASS 双探针；seed0/2 全门过；seed1 leakage_delta 5.75% > 5% 单 shuffle 门边际犯规）。多 shuffle 零假设分布仪器已落地（`leakage_probe(n_shuffles=k)`，k=1 与冻结行为逐位兼容 + `leakdist` 相位，8 项新测试，全套 126/126）；**冻结判决不改，待 Hermes 跑 leakdist k=16 × seed1（命令下附）出分布后再议 v0.4 门修正案**；修正案若立：判据改为"aware 改进超 shuffle 分布 95 分位 + delta_vs_mean > 5%"，先冻结后重判，三 seed 统一适用 |

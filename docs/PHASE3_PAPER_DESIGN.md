...
- 6.4 表示增强必须过方向证书（JEPA B 系列，seed0 探索批 9 臂）
  ...
  - 机制：慢态守恒功率修正注入与 valve2 喷水冷却共享守恒方程通道 → 机制级因果
    冲突；精度收益与因果破坏同源（同慢态注入路径）
  - 结论：表示增强收益必须与方向证书联合验证；动作盲化仅修复方向反转，
    无法消除效应稀释——问题在注入机理而非信息源
- 6.5 未来工作：多尺度损失与慢尺度假设
  - 现有慢态仍以步级观测 NLL 为损失 → 梯度迫使慢态拟合快动力学（含动作效应）
  - 量纲假设：慢态更新尺度 τ_slow 必须大于因果窗口上限（本机组 240-560s；
    现有 τ_slow=60s 偏快，慢态在 60s 尺度已可见喷水效应尾巴）
  - 提议设计：慢态专用损失（预测未来一个慢窗的粗粒化状态——负荷趋势/过热度
    趋势/工况标签，损失每慢窗计算一次）+ 快通路步级损失 → 表征按时间尺度分层
  - 成功判据：valve2 方向 frac ≥ c0 的 90% + 慢态探针与工况趋势显著相关

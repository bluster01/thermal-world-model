...
> **当前写作暂停点（2026-09-14）**：按作者要求，先由Linux完成 [CORE1真实数据同核对照](../../../../experiments/fmts_core_20260914/EVIDENCE_PLAN_ZH.md)，回传并审计后再整合本文。此处仅更新状态说明，正文/图/PDF数值不变；不提前宣称灰箱响应恢复或独立留出证据已补齐。

> **2026-09-14 补充结果已审计**：[完整结论及整合建议](../../audits/m7_gnr1_20260914/REVIEW_ZH.md)。M7 修复旧 token 的真实历史饱和，MAE 0.509880°C，但输给 GRU、0/3 不晋级；no-rewet 灰箱 MAE 0.973432°C，响应转负但仍弱且未校准。本目录 PDF/数值继续保留为 v0.2 历史快照，不冒充新结果版；下一版应纳入两项补充及边界，不能概括成 attention 更差或物理响应已正确。

本轮任务已收敛为：**主汽温精度与双阀动作响应的预测代价**。不是把 token 融合包装成胜出模型。
作者补充要求：纯灰箱的正向响应不能作为“物理响应正确”的依据。
已完成并审计 [GNR1 no-rewet 灰箱对照](../../../../experiments/fmts_greybox_norew_20260913/RESULTS.md)，只新增三个种子完整拟合。
**当前 PDF 是 v0.2 证据审读快照；尚未整合 9/14 的 M7/GNR1 返回结果，不是更新后的最终稿。**
...
锁定测试、现场响应校准及去 oracle 仍未授权执行；no-rewet 灰箱对照已回传审计，未验证现场增益正确。

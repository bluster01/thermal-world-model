...
**新叙事**: "我们构建了火电主汽温的世界模型, 验证了其预测能力, 发现了预测→控制的边界 (objective mismatch), 最终落地为预测驱动+监督模式"

| 维度 | 旧 | 新 |
|------|-----|-----|
| 定位 | 控制器替代 | 预测器+顾问 |
| 主结果 | MPC vs PID 闭环对比 | 开环预测精度 + 现场监督实证 |
| Phase 2 | 核心贡献 | 方法论探索 (发现边界) |
| 因果 | "无翻转" (错误) | 首步正确/持续反演 — Pearl L1 vs L2 |
...
- DirectWM: RevIN + PerVarTCN + VarAttn + 动作 concat, β-NLL
- 40维输入 → 1维目标 (末级过热器出口汽温)
- task-oriented 设计 (TD-MPC2 哲学): 只预测控制相关变量
- 引用: RevIN (Kim 2022), PatchTST (Nie 2023), β-NLL (Seitzer 2022), iTransformer (Liu 2024)
...
- **首步扰动** (exp_026b): M0 t12=−0.450, 方向正确 (Pearl L1: 干预)
- **持续阶跃** (S3): 2/3 起点 t12=+0.15~+0.23, 方向反演 (Pearl L2: 共因混杂)
- **诊断**: WM 学到条件期望 E[T|a], 不是因果效应 P(T|do(a))
- **符号正则 = 伪物理** (exp_016): 把 120s 滞后压成 10s 伪响应

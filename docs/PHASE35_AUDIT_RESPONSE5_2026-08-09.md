...
> **Supervisor 复核：本文件的“5A 完成”和“B 阀门无响应”已撤回。** v2 JSON 来自未提交的
> dirty tree，split payload/test lock/min-gap 仍有实现错误；B 侧在 3–30 s 有明显阀位反向响应，
> 约 50% 只对应 `dv_600s`。参数塌缩诊断保留。详见
> `PHASE3_5_LINUX_RETURN_AUDIT_2026-08-09.md`。

> 对应审计 §5"下一轮 Linux 只做七件事"的 1/2/6/7 项，以及 P0-1/P0-2/P0-3/P1 全部硬问题。
> 历史回传：A/B 侧 1s 事件完成一次探索运行；B 侧 600 s 净阀位异号率
> ~50%（A 侧 73-78%）——该数字只对应 600 s 净阀位变化；B 侧 3–30 s 早期响应反而明显，
> 不能据此下工程因果结论。G3 参数摘要显示 τ 接近上界（真实秒 1059-1189s）、gain/rate 塌缩。
...
但二级减温阀在 **600 s 净变化**上接近随机水平；3–10 s 异号率约 85–95%，说明早期执行存在、
随后发生回调或工况覆盖。不能据此断言温度变化不是通过二级阀通道实现。

A/B 存在 horizon-dependent 的观测差异（可能原因：B 侧减温阀投入率低、手动模式占比高、
...
| G4/G5 | HOLD | 无候选；A/B event test 均已 exploratory 暴露，模型 test 尚未评估 |

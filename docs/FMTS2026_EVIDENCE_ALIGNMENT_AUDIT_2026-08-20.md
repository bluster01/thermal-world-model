...
2. **修复批①-⑤已立项为矩阵修正案 v0.3**（用户 2026-08-20 晚批准；含 R1 规则修订与
   架构路线登记"gray-box split by time scale"），实施按 ①→⑤ 顺序逐批进矩阵重跑 T1+R1；
3. 侧 B 矩阵：**延至 AE/投稿补充阶段**（用户裁定），canonical_sideB.npz 已在仓；
4. 论文解冻按 §5.1，其中第 4 条按 single-side 声明路径执行（侧 B 不再先行）。

...
CFG_LIST = [
    # (tag, ASYMM_RATIO, LAMBDA_U, U_LO, U_HI)  — 安全带按数据 p5/p95 校准 (valve0 [0.2,43.6], valve1 [-0.75,32.6])
    ('base_sym',      1.0, 0.00, [2.0, 0.0], [43.0, 32.0]),   # 对称基线 = 主协议
    ('asym3',         3.0, 0.00, [2.0, 0.0], [43.0, 32.0]),
    ('asym5',         5.0, 0.00, [2.0, 0.0], [43.0, 32.0]),
    ('u_margin',      1.0, 0.10, [2.0, 0.0], [43.0, 32.0]),
    ('asym3_u',       3.0, 0.10, [2.0, 0.0], [43.0, 32.0]),
    ('asym3_u_tight', 3.0, 0.10, [5.0, 3.0], [40.0, 29.0]),   # 更紧安全带 (更强裕度)
]


def run_cfg(tag, asym, lam_u, u_lo, u_hi, n_tracks, smoke):
    M.ASYMM_RATIO = asym
    M.LAMBDA_U = lam_u
    ...
    out = {'tag': tag, 'asym': asym, 'lam_u': lam_u, 'u_lo': u_lo, 'u_hi': u_hi,
           'n_tracks': n_tracks, 'agg': agg, 'per_track': all_m}
...
def main():
    ...
    ap.add_argument('--asym', type=float, default=None)   # 单配置模式
    ...
    if args.asym is not None:  # 单配置
        cfg = [(args.tag, args.asym, args.lam_u or 0.0,
                args.u_lo if args.u_lo is not None else 5.0,
                args.u_hi if args.u_hi is not None else 95.0)]
    else:
    ...
    t0 = time.time()
    rows = []
    for tag, asym, lam_u, u_lo, u_hi in cfg:
        agg = run_cfg(tag, asym, lam_u, u_lo, u_hi, n_tracks, args.smoke)
        rows.append((tag, asym, lam_u, u_lo, u_hi, agg))
    # 汇总表
    print("\n===== 压线扫描汇总 =====")
    print(f"{'cfg':>16} | {'RMSE':>6} {'T̄mpc':>6} {'T̄pid':>6} {'mean_err':>8} "
          f"{'超温积分':>7} {'超温s':>5} {'TV':>5} {'viol':>4} {'margin':>6}")
    for tag, asym, lam_u, u_lo, u_hi, a in rows:

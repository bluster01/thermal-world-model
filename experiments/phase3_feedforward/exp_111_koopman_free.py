#!/usr/bin/env python3
"""
exp_111_koopman_free.py — Koopman free_head vs MLP free_head 对比实验
====================================================================
假设: Koopman 线性潜在动力学约束 free branch → 减少信号窃取
      → 干预分支 (InterventionPhysics) 获得更强梯度 → gain 提升

对比:
  A1phys        (baseline): MLP free_head + InterventionPhysics
  A1phys_koopman (new):     KoopmanFreeHead + InterventionPhysics

协议同 exp_106:
  W=96, H=60, BS=256, STEPS=500, AdamW LR 1e-3
  Smoke: 4 epochs, 足够看到趋势方向 (不追求全收敛)
"""
import os
import sys
import json
...
# ===================================================== 配置
VARIANTS_TO_RUN = ['A1phys', 'A1phys_koopman']
N_EPOCHS = 8            # smoke: 8 epochs (4 too few for any signal)
SEEDS = [0, 1, 2]       # 3 seeds
SMOKE = False            # full test (not --smoke which is 4 epochs)
P_FREEZE = 10            # ff10: 冻结 free 分支前10 epoch (P3A 最强策略)

OUT_DIR = 'results/exp_111_koopman_free'
...
def print_header():
    print("=" * 90)
    print(f"exp_111_koopman_free — Koopman vs MLP free_head 对比")
    print(f"  variants: {VARIANTS_TO_RUN}")
    print(f"  seeds: {SEEDS}")
    print(f"  epochs: {N_EPOCHS} (freeze-free={P_FREEZE})")
...
def main():
    ...
    # Koopman 特征值 (可解释性)
    print("\n" + "-" * 90)
    print("Koopman 特征值 (可解释性)")
    print("-" * 90)
    for i, r in enumerate(all_results.get('A1phys_koopman', [])):
        ckpt_path = os.path.join(
            OUT_ROOT,
            f'A1phys_koopman_s{SEEDS[i]}_ff{P_FREEZE}',
            'checkpoints', 'best_mae.pth'
        )
        if os.path.exists(ckpt_path):
            ckpt = torch.load(ckpt_path, map_location='cpu')
            # Load model and extract eigenvalues

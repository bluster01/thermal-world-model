#!/usr/bin/env python3
"""
exp_102_m9dsp_h18.py — M9(TimeXer) 架构 + ΔSP 动作 + H=18 (2026-08-05)
========================================================================
...
用法: python exp_102_m9dsp_h18.py [--smoke]
"""
...
for ep in range(1, NEPOCH + 1):
    ...
    if best_mae is None or te_mae < best_mae:
        ...
        os.makedirs('results/exp_102_m9dsp_h18/checkpoints', exist_ok=True)
        torch.save({'model_state_dict': model.state_dict(), 'epoch': ep},
                   'results/exp_102_m9dsp_h18/checkpoints/best_model.pth')
    curve.append((ep, losses / n, te_mae))

with open('results/exp_102_m9dsp_h18/metrics.json', 'w') as f:
    json.dump({'best_epoch': best_ep, 'best_test_mae': best_mae, 'curve': curve, 'H': H}, f, indent=2)
print(f"\n[done] best ep {best_ep} MAE {best_mae:.4f}°C")

# ===== 评测 (同 exp_100) =====
ck = torch.load('results/exp_102_m9dsp_h18/checkpoints/best_model.pth', map_location=DEVICE, weights_only=True)
...
print(f"\n===== 动作增益测试 (对比 exp_097 M5-DSP H=18: 响应0.05°C/方向75%; exp_101 M9DSP H=60 的180s段: 0.107/89%) =====")
PROFILE = ((3, '30s'), (6, '60s'), (12, '120s'), (17, '180s'))
for k, lab in PROFILE:
    r = resp_dT[:, k]
...
mono = np.abs(resp_dT[:, -1]).mean() > np.abs(resp_dT[:, 12]).mean() > np.abs(resp_dT[:, 6]).mean()
verdict = 'PASS' if np.abs(r_end).mean() >= 0.15 and dir_end >= 80 else 'FAIL'
print(f"  [判定] 180s 响应 {np.abs(r_end).mean():.3f}°C | 方向 {dir_end:.0f}% | 剖面单调 {mono} → {verdict}")

with open('results/exp_102_m9dsp_h18/gain_test.json', 'w') as f:
    json.dump({'resp_180s': float(np.abs(r_end).mean()), 'dir_180s': float(dir_end),
               'resp_profile': {lab: float(np.abs(resp_dT[:, k]).mean()) for k, lab in PROFILE},
               'dir_profile': {lab: float((np.sign(resp_dT[:, k]) == np.sign(dsp_v)).mean()) for k, lab in PROFILE},
               'mae_60': float(errs_all.mean()), 'mae_18': float(errs_18.mean()),
               'dir_60': float(dirs_all.mean()), 'monotone': bool(mono), 'verdict': verdict,
               'ref_m5dsp_h18': {'resp': 0.05, 'dir': 75.0, 'mae': 0.301},
               'ref_m9dsp_h60_180s': {'resp': 0.107, 'dir': 89.0}}, f, indent=2)
print('Saved: results/exp_102_m9dsp_h18/gain_test.json')

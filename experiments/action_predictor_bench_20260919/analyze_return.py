...
def analyze(root, destination):
    ...
    for folder in sorted(root.glob('seed*/*')):
        ...
        with np.load(folder / 'forecasts.npz') as predictions:
            for mode in ('block', 'native'):
                ...
                record = dict(model=result['name'], seed=result['seed'], mode=mode,
                    ...
                    upstream_max=max(x.get('unreachable_max_abs_C', 0) for x in rr),
                    pre_onset_max=max(x['pre_onset_max_abs_C'] for x in rr),
                    # Equal scenario weights; not a fraction pooled over all cells.
                    signed_scenario_mean_wrong_fraction=float(np.mean([x['opposite_sign_fraction'] for x in signed])),
    ...
    lines = ['# Returned baseline diagnosis', '',
             ...
             '|---|---|---:|---:|---:|---:|---:|---:|---:|']
    for x in rows:
        lines.append('| ' + ' | '.join([x['model'], x['mode'], *[f'{x[k]:.4f}' for k in

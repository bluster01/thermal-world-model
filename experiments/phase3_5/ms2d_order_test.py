#!/usr/bin/env python3
"""One-shot synthetic test access for frozen Phase 3.5-MS2-D2 checkpoints."""
...
TEST_PROTOCOL_VERSION = "phase3.5-ms2d-d2-test-v1"
DEFAULT_AUTHORIZATION = ROOT / "configs/phase3_5/ms2d_order_test_authorization.json"
...
def load_authorization(path: str | Path) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError(f"MS2-D2 test authorization missing keys: {missing}")
    if authorization["protocol_version"] != TEST_PROTOCOL_VERSION:
        raise ValueError(f"MS2-D2 test runner accepts only {TEST_PROTOCOL_VERSION}")
    expected_scope = {
    ...
    if authorization["authorized_scope"] != expected_scope:
        raise ValueError("MS2-D2 test authorization scope is not fail-closed")
    ...
    if int(authorization["expected_run_count"]) != 21:
        raise ValueError("MS2-D2 test authorization must freeze 21 runs")
    if int(authorization["checkpoint_archive"].get("member_count", 0)) != 21:
        raise ValueError("MS2-D2 checkpoint archive must contain 21 members")
    if int(authorization["test_samples"]) < 1:
        raise ValueError("MS2-D2 test_samples must be positive")
    bootstrap = authorization["bootstrap"]
    if int(bootstrap.get("replicates", 0)) < 1000:
        raise ValueError("MS2-D2 formal bootstrap requires at least 1000 replicates")
    if not isinstance(bootstrap.get("seed"), int):
        raise ValueError("MS2-D2 bootstrap seed must be an integer")
    expected_gates = {
        "oracle_clean_nmae_max",
        "order_aware_clean_nmae_max",
        "order_aware_response_ci_lower_min",
    }
    expected_diagnostics = {
        "tau_set_log_mae_max",
        "no_true_delay_expected_steps_max",
        "no_true_delay_zero_step_mass_min",
    }
    for label, payload, keys in (
        ("gates", authorization["gates"], expected_gates),
        ("diagnostics", authorization["diagnostics"], expected_diagnostics),
    ):
        if set(payload) != keys or any(
            not isinstance(value, (int, float)) or value < 0
            for value in payload.values()
        ):
            raise ValueError(f"MS2-D2 test {label} are incomplete or invalid")
...
def _assert_frozen_code_equivalent(training_sha: str) -> None:
    ...
    if exists.returncode != 0:
        raise RuntimeError(f"MS2-D2 training commit unavailable: {training_sha}")
    ...
    if compared.returncode == 1:
        raise RuntimeError(f"frozen MS2-D2 execution code differs from {training_sha}")
    if compared.returncode != 0:
        raise RuntimeError("git failed while checking frozen MS2-D2 code equivalence")


def _unauthorized_artifacts(run_dir: Path) -> list[str]:
    return [
        name
        for name in ("metrics_test.json", "episode_metrics_test.json", RUN_LEDGER_NAME)
...
def preflight(
    ...
    expected_gates = {
        ...
        "order_aware_clean_nmae_max": matrix["gates"][
            "order_aware_clean_nmae_max"
        ],
        "order_aware_response_ci_lower_min": matrix["gates"][
            "order_aware_relative_improvement_min"
        ],
    }
    expected_diagnostics = {
        "tau_set_log_mae_max": matrix["gates"]["tau_set_log_mae_max"],
        "no_true_delay_expected_steps_max": matrix["gates"][
            "no_true_delay_expected_steps_max"
        ],
        "no_true_delay_zero_step_mass_min": matrix["gates"][
            "no_true_delay_zero_step_mass_min"
        ],
    }
    if (
        _canonical(authorization["gates"]) != _canonical(expected_gates)
        or _canonical(authorization["diagnostics"])
        != _canonical(expected_diagnostics)
    ):
        raise RuntimeError("test gates or diagnostics differ from the frozen validation matrix")
    ...
    observed_status = {
        ...
        "order_aware_absolute_gate_pass": validation.get(
            "order_aware_absolute_gate", {}
        ).get("all_seeds_pass"),
        "order_aware_response_gate_pass": validation.get(
            "order_aware_response_gate", {}
        ).get("all_seeds_pass"),
        "tau_recovery_diagnostic_pass": validation.get(
            "tau_recovery_diagnostic", {}
        ).get("all_seeds_pass"),
        "no_true_delay_diagnostic_pass": validation.get(
            "no_true_delay_diagnostic", {}
        ).get("all_seeds_pass"),
        "all_primary_gates_pass": validation.get("all_primary_gates_pass"),
    }
    if observed_status != authorization["frozen_validation_status"]:
    ...
    if len(runs) != int(authorization["expected_run_count"]):
        raise RuntimeError("expanded MS2-D2 run count differs from authorization")
    ...
    if existing_root:
        raise RuntimeError(
            f"refusing repeat or partial MS2-D2 matrix test access: {existing_root}"
        )
    ...
    with tarfile.open(archive_path, "r") as archive:
        ...
        if len(members) != int(authorization["checkpoint_archive"]["member_count"]):
        ...
        observed_members = {member.name.replace("\\", "/") for member in members}
        ...
        for run in runs:
            ...
            if unauthorized:
                raise RuntimeError(
                    "refusing repeat or partial MS2-D2 run test access: "
            ...
            if mismatches:
                raise RuntimeError(
                    f"frozen MS2-D2 run mismatch for {candidate_id}/seed={seed}: "
                    f"{mismatches}"
                )
    ...
    if len(execution_shas) != 1:
        raise RuntimeError(f"MS2-D2 runs use multiple execution SHAs: {execution_shas}")
...
def execute_test_matrix(authorization_path: str | Path, device: str) -> dict[str, Any]:
    ...
    with tarfile.open(archive_path, "r") as archive:
        for index, record in enumerate(records, start=1):
            ...
            print(
                f"[{index}/{len(records)}] MS2-D2 test candidate={candidate_id} "
                f"seed={seed}",
                file=sys.stderr,
                flush=True,
            )
            ...
            checkpoint_bytes = _member_bytes(
                archive, record["checkpoint_member"], record["checkpoint_sha256"]
            )
            metrics, episodes = _evaluate_checkpoint_bytes(
                checkpoint_bytes, int(authorization["test_samples"]), device
            )
            ...
            manifest["test_access_note"] = "synthetic_order_known_truth_only"
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        status = authorization["frozen_validation_status"]
        print(
            json.dumps(
                {
                    "protocol_version": authorization["protocol_version"],
                    ...
                    "validation_screening_pass": status["all_primary_gates_pass"],
                    "tau_recovery_diagnostic_pass": status[
                        "tau_recovery_diagnostic_pass"
                    ],
                    "no_true_delay_diagnostic_pass": status[
                        "no_true_delay_diagnostic_pass"
                    ],
                    "test_accessed": False,
                },
                indent=2,
    ...
    if not args.evaluate_test_matrix:
        raise SystemExit("MS2-D2 test requires --evaluate-test-matrix")
    if not args.allow_synthetic_test:
        raise SystemExit("MS2-D2 test requires explicit --allow-synthetic-test")
    if authorization_path != DEFAULT_AUTHORIZATION.resolve():
        raise SystemExit("formal MS2-D2 test requires the repository authorization")

#!/usr/bin/env python3
"""Frozen RM3-AV 64-unit validation executor; unauthorized until Supervisor release."""
...
from src.phase35.multistep.rm3av_contracts import rm3av_run_specs, validate_rm3av_matrix
from src.phase35.multistep.rm3av_execution import run_rm3av_training


MATRIX = ROOT / "configs/phase3_5/ms3r_rm3av_matrix.json"
...
PINNED_CODE = (
    ...
    "experiments/phase3_5/ms3r_rm3av_train.py",
)
...
def _verify_registry() -> None:
    ...
    if registry.get("active_gate") != "ms3_r":
        raise RuntimeError("RM3-AV requires active_gate=ms3_r")
    if registry.get("linux_authorized_gate") != "ms3_r":
        raise RuntimeError("RM3-AV requires linux_authorized_gate=ms3_r")
    experiment = registry.get("experiments", {}).get("ms3_r", {})
    if experiment.get("status") != "ready_for_linux":
        raise RuntimeError("RM3-AV requires ms3_r.status=ready_for_linux")
    decision = experiment.get("decision", {})
    if decision.get("authorized_batch") != "RM3-AV0+AV1":
        raise RuntimeError("RM3-AV registry authorized_batch must equal RM3-AV0+AV1")
...
def _dirty_paths(output: Path) -> list[str]:
    ...
    return [
        ...
        if line.strip() and not line[3:].replace("\\", "/").startswith(allowed)
    ]
...
def _require_empty_output(output: Path) -> None:
    if output.exists() and any(output.iterdir()):
        raise FileExistsError(
            "RM3-AV refuses a non-empty output root; return partial artifacts to "
            "the Supervisor instead of resuming or retrying"
        )


def dry_run_payload(matrix: dict[str, Any]) -> dict[str, Any]:
    validate_rm3av_matrix(matrix, repo_root=ROOT)
    specs = rm3av_run_specs(matrix)
    return {
...
def _verify_run(output: Path, spec: Any, required: list[str]) -> dict[str, Any]:
    ...
    if set(required) != names:
        raise RuntimeError(f"RM3-AV artifact set changed for {spec.run_id}")
    ledger = _read(run_dir / "artifact_ledger.json")
    if set(ledger) != set(required) - {"artifact_ledger.json"}:
        raise RuntimeError(f"RM3-AV ledger fields changed for {spec.run_id}")
    for name, digest in ledger.items():
        if file_sha256(run_dir / name) != digest:
            raise RuntimeError(f"RM3-AV ledger hash mismatch for {spec.run_id}/{name}")
    ...
    if any(item.get("test_accessed") is not False for item in (manifest, metrics, diagnostics)):
        raise RuntimeError(f"RM3-AV validation artifact accessed test for {spec.run_id}")
    if manifest.get("run_id") != spec.run_id or metrics.get("candidate_id") != spec.candidate_id:
        raise RuntimeError(f"RM3-AV run identity drift for {spec.run_id}")
    if len(diagnostics.get("manual_audit_verdicts", {})) != 33:
        raise RuntimeError(f"RM3-AV verdict template incomplete for {spec.run_id}")
    return {
        "run_id": spec.run_id,
...
def _summarize(output: Path, specs: tuple[Any, ...], required: list[str]) -> dict[str, Any]:
    ...
    return {
        "training_unit_count": len(records),
        "all_runs_complete": len(records) == 64,
...
def execute(
    ...
    *,
    ...
    if require_clean and _dirty_paths(output):
        raise RuntimeError("RM3-AV requires a clean worktree")
    _require_empty_output(output)
    if not devices:
        raise RuntimeError("RM3-AV requires a non-empty device pool")
    caches = {"A": load_cache(cache_a), "B": load_cache(cache_b)}
    expected = matrix["data_contract"]["source_sha256"]
    for side in ("A", "B"):
        if (
            caches[side].metadata.get("side") != side
            or caches[side].metadata.get("source", {}).get("sha256") != expected
        ):
            raise RuntimeError(f"RM3-AV cache pin mismatch for {side}")
    specs = rm3av_run_specs(matrix)
    provenance = {
        ...
        "parent_independent_audit": matrix["parent_independent_audit"],
        "parent_design": matrix["parent_design"],
        "test_accessed": False,
    }
    ...
    _atomic(output / "run_manifest.json", {
        **provenance,
        ...
        "run_ids": [spec.run_id for spec in specs],
        "maximum_attempts_per_run": 1,
        "hermes_role": "execute_frozen_64_units_once_and_return_all_artifacts",
        "hermes_may_edit_code_or_configs": False,
        "hermes_may_retry_or_tune": False,
        "hermes_may_access_test": False,
        "automatic_scientific_pass": None,
    })
    partitions = [list(specs[index::len(devices)]) for index in range(len(devices))]
    records: list[dict[str, Any]] = []
    with ProcessPoolExecutor(
        max_workers=len(devices), mp_context=multiprocessing.get_context("spawn")
    ) as pool:
        futures = [
            pool.submit(
                _worker,
    ...
    complete = len(records) == 64 and all(row["status"] == "complete" for row in records)
    ...
    if complete:
        ...
        _atomic(
            ...
            build_root_artifact_ledger(
                output, matrix["execution_contract"]["required_root_artifacts"]
            ),
        )
    return records
...
def main() -> None:
    ...
    records = execute(
        matrix_path,
        Path(args.cache_a).resolve(),
        Path(args.cache_b).resolve(),

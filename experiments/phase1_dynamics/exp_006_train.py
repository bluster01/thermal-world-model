"""
exp_006_train.py — β-NLL 校准概率世界模型
===========================================
改动 vs exp_005:
- 损失: GaussNLL → BetaNLL (β=-0.5)
- β-NLL = σ^{2β} · NLL (stop-grad weight)
- 目的: 修复 σ 过自信（σ↓ 但 |error|↑）
...
EXP_META = {
    "exp_id": EXP_ID,
    "name": "β-NLL Calibrated Probabilistic World Model (warmup)",
    "date": datetime.now().isoformat(),
    "changes_vs_exp005": [
        f"损失: GaussNLL → BetaNLL (β={BETA}, warmup={BETA_WARMUP})",
        "warmup: 前20 epoch β=0 (标准NLL), 之后线性过渡到目标β",
        "β<0 惩罚小σ样本, 强制σ不过度坍缩",
    ],
    "rollout_k": ROLLOUT_K,
...
def main():
    ...
    # 输出对比
    print(f"\n{'='*70}")
    print(f"  {EXP_ID}: β-NLL Calibrated World Model (β={BETA})")
    print(f"{'='*70}")
    ...
    # vs exp_005 (GaussNLL, 无校准)
    old_sliding = [0.0772, 0.1098, 0.1603, 0.2512, 0.3960, 0.5582, 0.7857, 0.9528]
    old_steps = [0, 1, 2, 4, 7, 10, 14, 17]
    imp_sliding = []
    for s, m in zip(old_steps, old_sliding):
        imp_sliding.append((m - sliding_mae[s]) / m * 100)
    
    print(f"\n  vs exp_005 Sliding (GaussNLL, β=0):")
    ...
    results = {
        **EXP_META,
        ...
        'improvement_vs_exp005': {str(s): imp for s, imp in zip(old_steps, imp_sliding)},

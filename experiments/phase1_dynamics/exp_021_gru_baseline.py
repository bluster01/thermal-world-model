"""
exp_021_gru_baseline.py — GRU decoder 正确训练重测
====================================================
背景: exp_002-010 的"GRU decoder 死亡"结论有 bug — 训练循环用
model(x) (direct decoder), 而 GRU 分支 state_decoder_gru 从未被
梯度更新, 评测 mode='gru' 用的是随机权重 (MAE 恒定 1.3°C)。

本脚本: 与 exp_018 LSTM baseline 完全同协议, 只把 LSTM 换成 GRU:
- 同数据切分 (70/15/15) / 同绝对阀位 / 同 K=5 rollout loss / 同 β-NLL
- 同 seed 42 评测 (H=18 rollout + 敏感性 ±10)
- GRU 多层: 用 nn.GRU (支持 n_layers+dropout), 取最后时刻输出

用法: python exp_021_gru_baseline.py <A|B>
  A: GRU 含动作 | B: GRU 纯预测
"""
...
print(f"Config: {VARIANT} | include_action={INCLUDE_ACTION} | GRU h={GRU_HIDDEN} l={GRU_LAYERS} | K={ROLLOUT_K}")
...
class GRUDynamics(nn.Module):
    """GRU 动力学模型: [B, W, dim] → RevIN → GRU → MLP → (μ, logσ²)"""
    def __init__(self):
        super().__init__()
        d = cfg.N_STATE + (cfg.N_ACTION if INCLUDE_ACTION else 0)
        self.revin = RevIN(d)
        self.gru = nn.GRU(d, GRU_HIDDEN, GRU_LAYERS, dropout=cfg.DROPOUT, batch_first=True)
        self.head = nn.Sequential(
            nn.Linear(GRU_HIDDEN, 64), nn.GELU(), nn.Dropout(cfg.DROPOUT),
...
def main():
    model = GRUDynamics().to(DEVICE)
    print(f"Params: {sum(p.numel() for p in model.parameters()):,}")

    exp_dir = f"results/exp_021_{VARIANT}"
    ...
    result = {'config': VARIANT, 'include_action': INCLUDE_ACTION, 'model': 'GRU',
              'hidden': GRU_HIDDEN, 'layers': GRU_LAYERS, 'best_epoch': be,
